...
* 주 반사 에코(R) 이후 시간적으로 뒤따라오는 미약한 위성 신호(Satellite Pulse, s)가 회절파이며, 두 신호의 도달 시간 차(Δt)로 결함 깊이를 계산합니다.

	ext{결함 높이 } h = f(\Delta t, 	ext{음속}, 	ext{빔 각도})

* 표면 개구 결함(Surface Breaking Crack)의 경우 루트(하단)와 팁(상단) 각각에서 회절파가 발생하므로 두 신호의 Δt로 결함 높이를 직접 측정합니다.

### 4-2. 적용 조건 및 특성
* **진폭 비의존(Amplitude-Independent)**: 회절파의 도달 시간만 활용하므로 결함 방향이나 표면 상태의 영향을 덜 받습니다.
* **최적 각도**: 40^\circ \sim 50^\circ 경사각 탐촉자에서 정확도가 높습니다. 각도가 수평에 가까워질수록 팁-루트 간 Δt가 감소하여 측정이 어려워집니다.
...
### 5-1. 원리
* 팁 회절 원리를 체계화한 기법으로, 송신 탐촉자와 수신 탐촉자를 결함 양측에 배치하여 결함 상단(Upper Tip)과 하단(Lower Tip)의 회절파 도달 시간 차(Δt)로 결함 높이를 정밀 측정합니다.
    * **측면파 (Lateral Wave)**: 표면을 따라 직접 전달되는 파 (기준 시간 t_0로 사용)
