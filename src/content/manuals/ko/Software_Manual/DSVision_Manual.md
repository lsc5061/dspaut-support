...
- **부팅 즉시 실행**: 장비 전원을 켜면 운영체제 및 프로그램이 기동되며 곧바로 **실시간 측정 화면(예: PA-1 채널)**으로 자동 진입하여, 별도의 장비 연결 설정(Connection)이나 초기 빔 설정 마법사(Setup Wizard) 없이 신속한 현장 검사가 가능하도록 설계되어 있습니다.
- **설정 프리셋 관리 (.set)**: 상세 하드웨어 및 측정 환경 제어는 **Preferences (톱니바퀴) → Setup** 메뉴에서 수행하며, 탐촉자/웨지 규격, 빔 각도/해상도, 시험편 기하 형태, 인코더 설정 등이 프리셋 형태의 `.set` 파일 하나에 일괄 포함되어 내부/외부 USB 드라이브 간에 저장(Save), 로드(Load), 복사(Copy)가 가능합니다.
...
### 5.1.1 A-Scan 화면 표시 특징
- **축 구성**: 세로축은 시험체의 수직 깊이인 **Ultrasound 축 (Purple/Pink, 0~140mm)**이며, 가로축은 **신호 강도 (Amplitude, 0~100%)**를 나타냅니다.
- **햄버거 메뉴 (PA-1 타이틀 우측)**: 활성화된 채널 이름 옆의 햄버거 메뉴를 클릭하면 세 가지 파형 도구가 제공됩니다:
  - `Grid`: A-Scan 배경의 격자 눈금을 On/Off 토글합니다.
  - `Set Envelope`: 피크 홀드(Peak Hold) 기능으로, 실시간 파형들의 최대 진폭 궤적을 누적하여 화면에 겹쳐 표시합니다.
  - `Clear`: 누적된 Envelope 기록을 초기화합니다.
- **TCG 보정 곡선 상시 표출**: TCG(시간진폭보정) 완료 시, 캘리브레이션 모드를 벗어나도 A-Scan 배경에 청록색 보정 게인 곡선과 각 지점별 실측 가산 dB 수치(예: `5.9 dB`), 그리고 `Ref` 기준선이 상시 오버레이로 표출되어 신호 감쇠 보정이 안정적으로 구동 중임을 계속 모니터링할 수 있습니다.

### 5.1.2 S-Scan (부채꼴) 화면 표시 특징
- **축 및 기준점**: 하단 가로축은 **Index 축 (Green, -60mm~60mm)**입니다. 꼭짓점의 파란색 점은 초음파의 시작점인 탐촉자(Probe)의 기준점(Near Field) 위치를 나타냅니다.
- **초록색 세로 점선 (활성 라인)**: 현재 좌측 A-Scan에 표출 중인 대표 빔 각도를 가리킵니다.
  - 우측 하단의 `AScan Index` 화살표(◀▶)를 누르거나, 화면을 직접 터치 드래그하여 이동할 수 있습니다.
  - **각도 표시**: 조향 패널 자체에는 숫자가 표기되지 않고, S-Scan 화면 하단(예: `0.0°` 파란색 표시)에 현재 활성화된 빔의 굴절 각도가 실시간 표출됩니다.
- **Gate의 표시 방식**: 화면에 Gate 추가 시 에코 타입에 따라 다음과 같이 점선으로 표현됩니다.
  - *Sound Path 모드*: 호(Arc) 형태의 점선.
  - *True Depth 모드*: 수평 직선(Horizontal Line) 형태의 점선.
  - **Gate 색상 구분**: Gate A = **빨강(Red)**, Gate B = **파랑(Blue)**, Gate I = **초록(Green)**.
...
- **게이트 연동 전체 전환 규칙**: 상단 헤더 표시는 고정되지 않으며, 검사원이 선택(터치/활성화)한 특정 Gate(A, B, I)에 맞추어 헤더 측정 결과 약어와 수치가 통째로 전환됩니다 (예: Gate A 활성 시 PA/DA/SA, Gate B 활성 시 PB/DB/SB, Gate I 활성 시 PI/DI/SI).
- **Amplitude (%)**: 설정된 게이트 내에서 감지된 최대 에코(Echo) 신호의 강도(높이) 백분율을 의미합니다.
- **Primary (P - Projected Distance)**: 선택된 게이트 내에서 탐촉자 전방 방출점(Toe)으로부터 신호 감지 지점까지의 수평 투영 직선거리입니다.
- **Depth (D - True Depth)**: 시험체 표면으로부터 신호 감지 지점까지의 수직 깊이입니다.
- **Sound Path (S - Sound Path)**: 탐촉자로부터 빔이 실제 진행하여 신호 감지 지점까지 도달한 물리적 대각선 거리입니다.
...
- **동작 원리**: 현재 **Gate A** 영역 내에 위치한 신호 중 가장 높은 피크(Peak)를 자동으로 감지하여 증폭도를 조절합니다.
- **물리적 데시벨 계산 공식**: 실제 장비 내부에서는 다음 물리적 진폭비 공식에 의해 게인 보정량이 계산됩니다:
  $$\Delta\text{Gain} = 20 \log_{10}\left(\frac{\text{목표 \%}}{\text{현재 피크 \%}}\right)$$
  예를 들어, 현재 피크 높이가 40%이고 목표가 80%인 상황(2배 차이)일 때, 정확하게 **`+6 dB`**가 가산되어 물리 공식의 일관성이 그대로 검증되었습니다.
- **기준값 설정**: 우측 설정 메뉴의 **`Auto FSH` (Full Screen Height, 목표 %)** 목록에서 지정한 수치를 기준으로 신호 높이를 맞춥니다. ASME, AWS 등 비파괴검사 규격 교정에 주로 80% FSH가 표준으로 가장 많이 쓰이며, 상황에 따라 30%, 50% 등으로 가변 적용할 수 있습니다.
- **활용**: 수동으로 Gain 수치를 여러 번 조정할 필요 없이, 한 번의 클릭으로 신호를 규격에서 요구하는 기준 높이로 정규화할 수 있습니다.
